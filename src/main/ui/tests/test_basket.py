...
def tests_add_items_and_check_in_cart(page):
    catalog= CatalogSteps(page)
    basket = BasketSteps(page)
...
def test_checkout_multiple_items(page):
    ...
    basket.checkout()
    checkout.start_checkout(first_name='John', last_name='Daw',postal_code='660')
...
def test_checkout_without_item(page):
    catalog = CatalogSteps(page)
    basket = BasketSteps(page)
    checkout = CheckoutSteps(page)
    ...
    basket.checkout()
    checkout.start_checkout(first_name='John', last_name='Daw', postal_code='')
